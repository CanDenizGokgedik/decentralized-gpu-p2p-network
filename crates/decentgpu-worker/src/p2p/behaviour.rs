//! Worker node combined [`NetworkBehaviour`].
...
const PROTO_JOB: &str = "/decentgpu/job/1.0.0";

pub use job_codec::{JobCodec, JobRequest, JobResponse};

/// All P2P behaviours for the worker node.
...
pub struct WorkerBehaviour {
    ...
    /// Request-response: job assignment (master → worker, worker sends ack).
...
impl WorkerBehaviour {
    pub fn new(
        ...
        gossipsub.subscribe(&gossipsub::IdentTopic::new(TOPIC_HEARTBEAT)).expect("sub hb");

        let job_rr = request_response::Behaviour::new(
            vec![(StreamProtocol::new(PROTO_JOB), ProtocolSupport::Full)],
        ...
        Self {
            ...
            identify: identify::Behaviour::new(
                identify::Config::new("/decentgpu/identify/1.0.0".into(), key.public())
                    .with_agent_version("worker/0.1.0".into()),
            ),
...
pub mod job_codec {
    ...
    #[derive(Debug, Clone)] pub struct JobRequest(pub Vec<u8>);
    #[derive(Debug, Clone)] pub struct JobResponse(pub Vec<u8>);
    #[derive(Debug, Clone, Default)] pub struct JobCodec;
